...
// 16 result agents - used for roaming background
const AGENT_IMAGES = [
  'apple.png', 'bell.png', 'book.png', 'brackets.png', 'clippy.png',
  'formula.png', 'gear.png', 'infinity-glasses.png', 'lightbulb.png',
  'math.png', 'research.png', 'root.png', 'scribble.png', 'single-arrow.png',
  'time-schedule.png', 'umbrella.png'
];
  ...
  useEffect(() => {
    ...
    leaderboardData.forEach(entry => {
      ...
      for (let i = 0; i < entry.count; i++) {
        ...
        // Get the correct agent image filename - matches 16 result agents
        const getAgentImageFilename = (agentName: string): string => {
          const nameToFile: { [key: string]: string } = {
            'Ghost': 'infinity-glasses.png', 'Triangle': 'research.png', 'File': 'formula.png', 'Check': 'time-schedule.png',
            'Hat': 'math.png', 'Plug': 'brackets.png', 'Alarm': 'root.png', 'Book': 'book.png',
            'Lightbulb': 'lightbulb.png', 'Mailbox': 'bell.png', 'Rock': 'single-arrow.png', 'Sign': 'gear.png',
            'Chat': 'scribble.png', 'Puzzle': 'clippy.png', 'Umbrella': 'umbrella.png', 'Apple': 'apple.png'
          };
          return nameToFile[agentName] || agentName.toLowerCase().replace(/\s+/g, '-') + '.png';
        };
        ...
        <body>
          ...
          <script>
            ...
            const getAgentImageFilename = (agentName) => {
              ...
              return nameToFile[agentName] || agentName.toLowerCase().replace(/\\s+/g, '-').replace('/', '-') + '.png';
            };
            
            const getAgentCardImage = (agentName) => {
              const nameMapping = {
                'Ghost': 'Ghost.png', 'Triangle': 'Triangle.png', 'File': 'File.png', 'Check': 'Check.png',
                'Hat': 'Hat.png', 'Plug': 'Plug.png', 'Alarm': 'Alarm.png', 'Book': 'Book.png',
                'Lightbulb': 'Lightbulb.png', 'Mailbox': 'Mailbox.png', 'Rock': 'Rock.png', 'Sign': 'Sign.png',
                'Chat': 'Chat.png', 'Puzzle': 'Puzzle.png', 'Umbrella': 'Umbrella.png', 'Apple': 'Apple.png'
              };
            ...
            const openAgentModal = (agentName) => {
              ...
              image.src = getAgentCardImage(agentName) + '?v=2';
            ...
            agents.forEach(agent => {
              ...
              img.src = '/images/agents/' + agent.image + '?v=2';
              img.alt = 'Agent';
            ...
            const populateLeaderboard = () => {
              ...
              leaderboardData.forEach((entry, index) => {
                ...
                agentImg.src = '/images/agents/' + getAgentImageFilename(entry.agent) + '?v=2';
  ...
  return (
    <div className={styles.miniRoamingContainer} ref={containerRef}>
      ...
      {miniAgents.map(agent => (
        ...
        >
          <img
            src={`/images/agents/${agent.image}?v=2`}
            alt="mini agent"
...
function BackgroundRoamingAgents({ agentName }: { agentName: string }) {
  ...
  // Get agent image - 16 result agents
  const getAgentImagePath = (name: string): string => {
    ...
    const filename = nameToFile[name] || name.toLowerCase().replace(/\s+/g, '-') + '.png';
...
function GravityHomepage({ onStartTest }: { onStartTest: () => void }) {
  ...
  useEffect(() => {
    const initializeAgents = () => {
      ...
      const maxAgents = isMobile ? 10 : AGENT_IMAGES.length; // 10 on mobile, 16 on desktop
      ...
      <div className={styles.roamingOverlay}>
        ...
        >
          ...
          <p className={styles.description}>
            Answer 10 questions to reveal what Notion agent<span className={styles.desktopBreak}><br /></span> aligns most closely with your personality.
          </p>
...
// Question dimension mapping (4 binaries: I/T, S/U, A/C, D/Y)
const questionDimensions: { [key: number]: string } = {
  ...
  3: 'SU',  // Question 4: Vacation planning
  4: 'TI',  // Question 5: Work style preference
  5: 'TI',  // Question 6: My ideal day
  6: 'AC',  // Question 7: Success definition
  7: 'SU',  // Question 8: Project planning
  8: 'AC',  // Question 9: Co-worker stress
  9: 'YD'   // Question 10: Lead vs Harmony
};

// Calculate final personality type using majority vote for multi-question dimensions (4 binaries: I/T, S/U, A/C, D/Y)
const calculatePersonalityType = (answers: { [key: number]: string }): string => {
  ...
  // Build the final personality sequence: I/T, S/U, A/C, D/Y
  return finalLetters['TI'] + finalLetters['SU'] + finalLetters['AC'] + finalLetters['YD'];
};

const agents: { [key: string]: AgentProfile } = {
  "IUAD": {
    name: "Ghost",
    image: "infinity-glasses.png",
    strengths: [
...
export default function NotionAgentTest() {
  ...
  const getAgentImage = (agentName: string) => {
    // Map 16 result agents to agent image filenames
    const nameToFile: { [key: string]: string } = {
      'Ghost': 'infinity-glasses.png', 'Triangle': 'research.png', 'File': 'formula.png', 'Check': 'time-schedule.png',
      'Hat': 'math.png', 'Plug': 'brackets.png', 'Alarm': 'root.png', 'Book': 'book.png',
      'Lightbulb': 'lightbulb.png', 'Mailbox': 'bell.png', 'Rock': 'single-arrow.png', 'Sign': 'gear.png',
    ...
    const filename = nameToFile[agentName] || agentName.toLowerCase().replace(/\s+/g, '-') + '.png';
    return `/images/agents/${filename}`;
  };

  const getAgentCardImage = (agentName: string): string => {
    // Map agent names to result card images in /images/agent-cards/ (16 results)
  ...
  const getBinaryChoices = (sequence: string) => {
    const [first, second, third, fourth] = sequence.split('');
    
    return {
      collaboration: first === 'T' ? 'Collaborative' : 'Independent',
      structure: second === 'S' ? 'Structured' : 'Unstructured',
      thinking: third === 'A' ? 'Abstract' : 'Concrete',
      approach: fourth === 'D' ? 'Impact' : 'Harmony'
    };
  };
  ...
  const generateCompleteLeaderboard = (apiLeaderboard: LeaderboardEntry[]): LeaderboardEntry[] => {
    const allAgentNames = Object.values(agents).map(agent => agent.name);
    const validEntries = apiLeaderboard.filter(entry => allAgentNames.includes(entry.agent));
    const apiDataMap = new Map(validEntries.map(entry => [entry.agent, entry.count]));
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      if (data.leaderboard) {
        ...
        const totalFrom16 = completeLeaderboard.reduce((sum, entry) => sum + entry.count, 0);
        setTotalResults(totalFrom16);
        console.log(`Complete Leaderboard: ${totalFrom16} total, ${completeLeaderboard.length} agents (${data.leaderboard.length} with results from API)`);
      }
    } catch (error) {
            ...
            {leaderboardView === 'detail' && selectedAgent ? (
              <div className={styles.agentDetailHeader}>
                <div className={styles.breadcrumb}>
                  <button type="button" className={`${styles.backButton} ${styles.breadcrumbBackButton}`} onClick={backToLeaderboard} aria-label="Back to results">
                ...
                <>
                  ...
                  <div className={styles.agentDetailContent}>
                    <div className={styles.agentDetailCard}>
                      <img
                        src={`${getAgentCardImage(selectedAgent)}?v=2`}
                        ...
                        <div key={entry.agent} className={`${styles.leaderboardItem} ${isZeroResults ? styles.zeroResults : ''}`}>
                          <div className={styles.statsInfo}>
                            <div className={styles.rankInfo} onClick={() => openAgentDetail(entry.agent)}>
                              <img 
                                src={`${getAgentImage(entry.agent)}?v=2`}
                                alt={entry.agent}
